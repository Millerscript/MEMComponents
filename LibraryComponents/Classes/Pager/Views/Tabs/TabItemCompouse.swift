...
class TabItemCompouse: UIView, TabProtocol {
    ...
    func set(data: MCTabItemModel) {
        setTitle(text: data.text, image: data.icon!)
    }
    
    private func setTitle(text: String, image: UIImage) {
        tabItemTitle.bind()
        self.tabItemTitle.setData(text: text, icon: image)
        self.tabItemTitle.action = { [self] in
            action.send(TabData(index: index, type: .COMPOUSE))
        }
    }
